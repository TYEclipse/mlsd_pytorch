...
class Simple_MLSD_Learner():
    def __init__(self,
        ...
        self.score_threshold = self.cfg.decode.score_thresh
        self.top_k = self.cfg.decode.top_k
        self.length_threshold = self.cfg.decode.len_thresh
    ...
    def val(self, model, val_dataloader: DataLoader, score_threshold=0.5, topk=100, length_threshold=10, sap_threshold=10):
        model = model.eval()
        ...
        with torch.no_grad():
            for batch_data in data_iter:
                ...
                for outputs, gt_lines_list_all in zip(batch_outputs, batch_data["gt_lines_512"]):
                    ...
                    tp, fp = msTPFP(pred_lines_128, gt_lines_128, sap_threshold)
    ...
    def train(self, train_dataloader: DataLoader,
        ...
        for self.epo in range(epoches):
            ...
            for batch in data_iter:
                ...
                train_avg_center_loss.update(
                    loss_dict['center_loss'].item(), 1)
                train_avg_replacement_loss.update(
                    loss_dict['displacement_loss'].item(), 1)
                train_avg_line_seg_loss.update(
                    loss_dict['line_seg_loss'].item(), 1)
                train_avg_junc_seg_loss.update(
                    loss_dict['junc_seg_loss'].item(), 1)
