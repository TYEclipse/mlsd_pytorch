...
def weighted_bce_with_logits(out, gt, pos_w=1.0, neg_w=30.0):
    ...
    loss_pos = ((loss * pos_mask).sum()+1) / (torch.sum(pos_mask)+1)
    loss_neg = ((loss * neg_mask).sum()+1) / (torch.sum(neg_mask)+1)
...
def len_and_angle_loss_func(pred_len, pred_angle, gt_len, gt_angle):
    ...
    # only consider non zero part
    pos_mask = torch.where(gt_len != 0, torch.ones_like(
        gt_len), torch.zeros_like(gt_len))
    pos_mask_sum = pos_mask.sum()
...
def displacement_loss_func(pred_dis, gt_dis, gt_center_mask=None):
    ...
    pos_mask = torch.where(
        pos_v != 0, torch.ones_like(x0), torch.zeros_like(x0))
    pos_mask_sum = pos_mask.sum()
    ...
    displacement_loss1 = F.smooth_l1_loss(
        pred_dis, gt_dis, reduction='none').sum(axis=[1])

    # swap pt
    pred_dis2 = torch.cat(
        (pred_dis[:, 2:, :, :], pred_dis[:, :2, :, :]), dim=1)
    displacement_loss2 = F.smooth_l1_loss(
        pred_dis2, gt_dis, reduction='none').sum(axis=[1])
    displacement_loss = displacement_loss1.min(displacement_loss2)
    # if gt_center_mask is not None:
...
class LineSegmentLoss(nn.Module):
    ...
    def _m_gt_matched_n(self, p_lines, gt_lines, thresh):
        ...
        distance1 = F.pairwise_distance(
            gt_lines[:, :2], matched_pred_lines[:, :2], p=2)
        distance2 = F.pairwise_distance(
            gt_lines[:, 2:], matched_pred_lines[:, 2:], p=2)
    ...
    def _m_match_loss_fn(self, p_lines, p_centers, p_scores, gt_lines, thresh):
        ...
        # 找出距离最近的预测直线
        distance1 = F.pairwise_distance(
            matched_gt_lines[:, :2], p_lines[:, :2], p=2)
        distance2 = F.pairwise_distance(
            matched_gt_lines[:, 2:], p_lines[:, 2:], p=2)
        inx = torch.where((distance1 < thresh) & (distance2 < thresh))[0]
        ...
        if match_n > 0:
            ...
            endpoint_loss = F.l1_loss(
                mathed_pred_lines, mathed_gt_lines, reduction='mean')  # * 2

            gt_centers = (mathed_gt_lines[:, :2] + mathed_gt_lines[:, 2:]) / 2
            center_dis_loss = F.l1_loss(
                mathed_pred_centers, gt_centers, reduction='mean')
    ...
    def matching_loss_func(self, pred_tp_mask, gt_line_512_tensor_list):
        ...
        for pred, gt_line_512 in zip(pred_tp_mask, gt_line_512_tensor_list):
            ...
            pred_center_ptss_128 = 128 * \
                pred_center_ptss / (self.input_size / 2)

            pred_lines_128 = torch.cat(
                (pred_lines_128, pred_lines_128_swap), dim=0)
            pred_center_ptss_128 = torch.cat(
                (pred_center_ptss_128, pred_center_ptss_128), dim=0)
            ...
            match_n = self._m_gt_matched_n(
                pred_lines_128, gt_line_128, self.match_sap_thresh)
    ...
    def tp_mask_loss(self, out, gt, gt_lines_tensor_512_list):
        ...
        if self.with_focal_loss:
            ...
            center_loss = weighted_bce_with_logits(
                out_center, gt_center, 1.0, 30.0)

        out_displacement = out[:, 8:12, :, :]
        gt_displacement = gt[:, 8:12, :, :]
        displacement_loss = displacement_loss_func(
            out_displacement, gt_displacement, gt_center)
    ...
    def line_and_juc_seg_loss(self, out, gt):
        ...
        if self.with_focal_loss and self.focal_loss_level >= 3:
            line_seg_loss = focal_neg_loss_with_logits(
                out_line_seg, gt_line_seg)
        else:
            line_seg_loss = weighted_bce_with_logits(
                out_line_seg, gt_line_seg, 1.0, 1.0)
        out_junc_seg = out[:, 14, :, :]
        gt_junc_seg = gt[:, 14, :, :]
        if self.with_focal_loss and self.focal_loss_level >= 2:
            junc_seg_loss = focal_neg_loss_with_logits(
                out_junc_seg, gt_junc_seg)
        else:
            junc_seg_loss = weighted_bce_with_logits(
                out_junc_seg, gt_junc_seg, 1.0, 30.0)
    ...
    def forward(self, preds, gts,
        ...
        loss_dict['loss'] = loss
        if self.with_SOL_loss:
            loss_dict['center_loss'] = loss_dict['sol_center_loss'] + \
                loss_dict['tp_center_loss']
            loss_dict['displacement_loss'] = loss_dict['sol_displacement_loss'] + \
                loss_dict['tp_displacement_loss']
            loss_dict['match_loss'] = loss_dict['tp_match_loss'] + \
                loss_dict['sol_match_loss']
